    ...
    <div className="flex flex-wrap items-end gap-3">
      ...
      <div>
        ...
        <select value={status} onChange={(e) => navigate({ status: e.target.value || undefined })} className={`${inputClass} mt-1`}>
          ...
          {/* "interview_scheduled" isn't offered as something to filter to
              from here (see SETTABLE_APPLICATION_STATUSES) — but the
              Dashboard's own "Interview Scheduled" tile still links to
              ?status=interview_scheduled, and that still needs to display
              correctly if landed on directly, not just silently mismatch. */}
          {status === "interview_scheduled" && (
            <option value="interview_scheduled">{APPLICATION_STATUS_LABELS.interview_scheduled}</option>
          )}
