      ...
      <header className="sticky top-0 z-10 flex shrink-0 flex-col border-b border-black/[0.06] bg-white/80 backdrop-blur-xl print:hidden">
        ...
        <div className="flex items-center px-5 py-4">
          <Link href="/dashboard" className="flex items-center gap-4">
            ...
            <div>
              <span className="hidden text-3xl font-bold tracking-tight text-slate-900 sm:block">{branding.name}</span>
              <span className="block text-2xl font-bold tracking-tight text-slate-900 sm:hidden">{branding.shortName}</span>
              {branding.tagline && <p className="mt-0.5 text-[14px] text-slate-500">{branding.tagline}</p>}
