...
export async function scheduleInterview(formData: FormData) {
  ...
  try {
    ...
    const cc = parseEmailList(application.tenant.interviewEmailCc);
    const bcc = parseEmailList(application.tenant.interviewEmailBcc);
    const result = await sendEmail({ to: application.candidate.email, toName: application.candidate.fullName, cc, bcc, subject, html });
