...
export type FindRemovedResult = { supported: true; removed: RemovedApplication[] } | { supported: false; reason: string };

// syncTenantSheet only ever adds/updates — a row deleted from the Sheet
// leaves its application untouched, on purpose (see the file-level
// comment: safe to run on a schedule, never silently loses workflow
// state). This is the read-only counterpart for the case an admin
// actually wants: "which applications no longer have a matching row in
// the Sheet?" Nothing is deleted here — the caller reviews the list and
// deletes whichever ones they confirm via a separate action.
//
// Only meaningful when coreFields.applicationNumberCol is set (the Sheet
// assigns each row its own stable ID). Without it, a row's position IS
// its identity — deleting one row shifts every later row's derived
// number too, which would make most/all of them look "removed" even
// though the candidates are still there. Returns an explicit
// unsupported result for that case instead of a misleading list.
export async function findApplicationsNotInSheet(prisma: PrismaClient, tenantSlug: string): Promise<FindRemovedResult> {
  const tenant = await prisma.tenant.findUniqueOrThrow({ where: { slug: tenantSlug } });
  if (!tenant.sheetSourceUrl || !tenant.sheetMappingJson) {
    throw new Error(`Tenant "${tenantSlug}" has no sheetSourceUrl/sheetMappingJson configured — nothing to check.`);
  }
  const config = parseSheetImportConfig(tenant.sheetMappingJson);
  const { applicationNumberCol } = config.coreFields;
  if (applicationNumberCol === null || applicationNumberCol === undefined) {
    return {
      supported: false,
      reason:
        "This client's Sheet has no unique ID column configured, so a row's position is its only identity — deleting a row shifts every later row's number too, which would make this check unreliable. Not available for this client's current setup.",
    };
  }
  ...
  const currentNaturalNumbers = new Set<string>();
  for (const row of dataRows) {
    const rawId = cell(row, applicationNumberCol);
    if (rawId) currentNaturalNumbers.add(`${config.applicationNumberPrefix}${rawId}`);
  }
  ...
  // Mirrors syncTenantSheet's own "already imported" membership check
  // (see its collision-handling comment above) so this can never diverge
  // from what the sync engine itself considers a match: a stored number
  // is either exactly one of the Sheet's current natural numbers, or a
  // disambiguated "NATURAL-N" suffix of one.
  const removed = applications.filter((app) => {
    for (const natural of currentNaturalNumbers) {
      if (app.applicationNumber === natural || app.applicationNumber.startsWith(`${natural}-`)) return false;
    }
    return true;
  });
